...
void Shape::loadMesh(const string &meshName, bool texOn)
{
	loadMesh(meshName);
	if (!texOn) {
		texBuf.clear();
...
void Shape::moveToGround() {
	...
	//Go through all vertices to determine min and max Y's
	for (size_t v = 0; v < posBuf.size() / 3; v++) {
		if (posBuf[3 * v + 1] < minY) minY = posBuf[3 * v + 1];
		if (posBuf[3 * v + 1] > maxY) maxY = posBuf[3 * v + 1];
	}

	//Go through all verticies and shift
	for (size_t v = 0; v < posBuf.size() / 3; v++) {
		posBuf[3 * v + 1] -= minY;
	}
}
...
void Shape::print() {
	...
	cout << "Normals" << endl;
	for (unsigned int i = 0; i < norBuf.size(); i++) {
		if (i % 3 == 0 && i != 0) {
			cout << "," << endl;
		}
		cout << norBuf[i] << " ";
	...
	cout << "Elements" << endl;
	for (unsigned int i = 0; i < eleBuf.size(); i++) {
		if (i % 3 == 0 && i != 0) {
			cout << "," << endl;
		}
		cout << eleBuf[i] << " ";
...
Vector3f Shape::getCenter() {
	unsigned int numVertices = posBuf.size() / 3;
